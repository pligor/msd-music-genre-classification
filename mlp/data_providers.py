...
class MSD10GenreDataProvider(OneOfKDataProvider):
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, data_filtering = lambda inputs, targets: (inputs, targets), initial_order = None):
...
class MSD10Genre_Autoencoder_DataProvider(MSD10GenreDataProvider):
    """Simple wrapper data provider for training an autoencoder on MNIST."""

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(MSD10Genre_Autoencoder_DataProvider, self).next()
        # return inputs as targets for autoencoder training
        return inputs_batch, inputs_batch


class MSD10Genre_StackedAutoEncoderDataProvider(MSD10GenreDataProvider):
    """Wrapper data provider for training a stacked autoencoder"""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, processBatch=None):
        """
        processBatch should be a function which fprops the input batch all the already pretrained layers of the stack
        """
        super(MSD10Genre_StackedAutoEncoderDataProvider, self).__init__(
            which_set, batch_size, max_num_batches, shuffle_order, rng)
        self.processBatch = processBatch

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputsBatch, targetsBatch = super(MSD10Genre_StackedAutoEncoderDataProvider, self).next()
        # return inputs as targets for autoencoder training

        processedBatch = inputsBatch if self.processBatch is None else self.processBatch(inputsBatch)

        #print "processedBatch.shape: " + str(processedBatch.shape) #bad idea, too many outputs

        return processedBatch, processedBatch
...
class MSD25Genre_StackedAutoEncoderDataProvider(MSD25GenreDataProvider):
    """Wrapper data provider for training a stacked autoencoder"""
	...
	processBatch should be a function which fprops the input batch all the already pretrained layers of the stack
        """
        super(MSD25Genre_StackedAutoEncoderDataProvider, self).__init__(
		which_set, batch_size, max_num_batches, shuffle_order, rng)
        self.processBatch = processBatch

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputsBatch, targetsBatch = super(MSD25Genre_StackedAutoEncoderDataProvider, self).next()
        processedBatch = inputsBatch if self.processBatch is None else self.processBatch(inputsBatch)
        return processedBatch, processedBatch
...
class StackedAutoEncoderDataProvider(MNISTDataProvider):
    """Simple wrapper data provider for training an autoencoder on MNIST."""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, model=None):
        """
        :param model: MultipleLayerModel
        """
        super(StackedAutoEncoderDataProvider, self).__init__(
            which_set, batch_size, max_num_batches, shuffle_order, rng)
        self.model = model
        self.test = False
    ...
    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputsBatch, targetsBatch = super(StackedAutoEncoderDataProvider, self).next()
        # return inputs as targets for autoencoder training

        processedBatch = inputsBatch if self.model is None else self.model.fprop(inputsBatch)[-1]

        if self.test:
            print processedBatch.shape
            self.test = False

        return processedBatch, processedBatch


class MNISTAutoencoderWithNoiseDataProvider(MNISTDataProvider):
    """Simple wrapper data provider for training an autoencoder on MNIST."""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, std=0.3):
        super(MNISTAutoencoderWithNoiseDataProvider, self).__init__(
            which_set, batch_size, max_num_batches, shuffle_order, rng)
        self.std = std

    @staticmethod
    def addGaussianNoise(vectors, std, rng):
        return vectors + std * rng.randn(vectors.shape[0], vectors.shape[1])

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(MNISTAutoencoderWithNoiseDataProvider, self).next()
        # return inputs as targets for autoencoder training
        return MNISTAutoencoderWithNoiseDataProvider.addGaussianNoise(inputs_batch, self.std, self.rng), inputs_batch
